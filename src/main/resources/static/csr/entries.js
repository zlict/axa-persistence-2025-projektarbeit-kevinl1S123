    ...
    fetch(`${URL}/entries`, {
      ...
      if (result.status < 300) {
        result.json().then((entry) => {
          ...
          if (cat) entry.category.name = cat.name;
          if (entry.tags && entry.tags.length > 0) {
            entry.tags = entry.tags.map(t => {
              const tagObj = tags.find(tag => tag.id == t.id);
              return tagObj ? { ...t, name: tagObj.name } : t;
            });
          }
          entries.push(entry);
          renderEntries();
...
const editEntry = (id) => {
  ...
  const duration = new Date(`${entry.duration}Z`);
  ...
  entryForm.querySelector('input[name=duration]').value = entry.duration;
  entryForm.querySelector('input[name=description]').value = entry.description;
};
...
const renderEntries = () => {
  ...
  entries.forEach((entry) => {
    ...
    row.appendChild(createCell(entry.category && entry.category.name ? entry.category.name : '')); // Kategorie-Name anzeigen
